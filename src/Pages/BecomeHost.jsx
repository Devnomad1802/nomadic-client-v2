...
/**
 * BecomeHost.jsx — host registration form (design: "Become a Host").
 * Two-panel: left brand image panel, right form. Submits as an enquiry via
 * useEnquirMutation. Reached from "Become a Host" in Meet Our Hosts.
 */
...
const BecomeHost = () => {
  ...
  const submit = async (e) => {
    ...
    if (Object.keys(er).length) {
      const first = document.querySelector(`[name="${Object.keys(er)[0]}"]`);
      first?.focus();
      return;
    }
    const cat = form.category === "Other" ? form.categoryOther : form.category;
    const Message = [
      "Become a Host application",
      `Category: ${cat}`,
      `City/Country: ${form.city}`,
      `Experience: ${form.years}`,
    ...
    try {
      await enquir({ Name: form.fullName, Email: form.email, Phone: form.mobile, Message, userId: userDbData?._id }).unwrap();
    } catch { /* show success regardless — team follows up */ }
    setDone(true);
    window.scrollTo(0, 0);
  };
  ...
  const field = (name, label, props = {}) => (
    <div className={`bh-fld${props.full ? " full" : ""}`}>
      <label>{label} {props.optional && <span className="opt">· optional</span>}</label>
  ...
  return (
    <div className="bhpg">
      <Helmet>
        <title>Become a Host | Nomadic Townies</title>
        <meta name="description" content="Join Nomadic Townies as a host — share homestays, treks, workshops and local experiences with travellers. Apply in minutes, no fees." />
        <link rel="canonical" href="https://nomadictownies.com/become-a-host" />
      </Helmet>
            ...
            <div className="bh-success">
              <div className="bh-success-in">
                ...
                <div className="bh-success-actions">
                  <button className="bh-btn-light" onClick={() => navigate("/all-packages")}>Explore experiences</button>
